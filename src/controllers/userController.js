...
export const getOauthUrlSignUp = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao gerar url para autenticação Oauth" });
  }
};
    ...
    req.session.regenerate((err) => {
      ...
      req.session.save((err) => {
        ...
        addUserSession(req.session.user, req.sessionID).catch((e) => {
          logger.error("Erro ao gravar session id no Redis", e, {
...
export const signUp = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Para concluir o registro, verifique seu email" });
  } catch (error) {
  ...
  try {
    ...
    req.session.regenerate((err) => {
      ...
      req.session.save((err) => {
        if (err) {
          logger.error(
            "Erro ao salvar sessão após a verificação de email",
            err,
            {
              usuarioId: service._id,
              ip,
