...
export const addUserSession = async (userId, sessionId) => {
  try {
    ...
    logger.debug("Sessão adicionada ao conjunto do usuário no Redis", {
      usuarioId: userId,
      sessionId,
    });
  } catch (error) {
    logger.error("Erro ao adicionar sessão do usuário no Redis", error, {
      usuarioId: userId,
      sessionId,
    });
  }
...
export const removeUserSession = async (userId, sessionId) => {
  try {
    ...
    logger.debug("Sessão removida do conjunto do usuário no Redis", {
      usuarioId: userId,
      sessionId,
    });
  } catch (error) {
    logger.error("Erro ao remover sessão do usuário no Redis", error, {
      usuarioId: userId,
      sessionId,
    });
  }
...
export const revokeAllUserSessions = async (userId) => {
  try {
    ...
    if (!sessions || sessions.length === 0) {
      logger.debug("Nenhuma sessão encontrada para revogação", {
        usuarioId: userId,
      });
      return;
    }
    ...
    logger.info("Sessões do usuário revogadas com sucesso", {
      usuarioId: userId,
      revokedSessionsCount: sessions.length,
    });
  } catch (error) {
    logger.error("Erro ao revogar sessões do usuário no Redis", error, {
      usuarioId: userId,
    });
    throw error;
  }
