...
export const findUserByEmail = async (email) => {
  return await User.findOne({ email })
    .select(
      "email name password _id autenticationType isVerified sessionVersion",
    )
    .lean();
};
...
export const VerifyEmailExists = async (email) => {
  return await User.findOne({ email }).select("email").lean();
};
...
export const findUserByIdVerified = async (id) => {
  return await User.findById(id).select("isVerified sessionVersion").lean();
};
