...
export const signInValidate = (req, res, next) => {
  ...
  switch (error?.details[0].message) {
    case `"password" with value "${password}" fails to match the required pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,}$/`:
      logger.warn(
        `IP ${req.ip} tentou fazer login com senha que não atende os requisitos de complexidade`,
        {
    ...
    case '"email" length must be at least 13 characters long':
      ...
      return res
        .status(400)
        .json({ error: "O email deve conter no mínimo 13 caracteres" });
    case '"email" length must be less than or equal to 50 characters long':
      ...
      return res
        .status(400)
        .json({ error: "O email deve conter no máximo 50 caracteres" });
    ...
    case '"password" length must be less than or equal to 100 characters long':
      ...
      return res
        .status(400)
        .json({ error: "A senha deve conter no máximo 100 caracteres" });
    case '"password" must only contain alpha-numeric characters':
      ...
      return res
        .status(400)
        .json({ error: "A senha deve conter apenas caracteres alfanuméricos" });
    case '"email" must be a valid email':
      ...
      return res
        .status(400)
        .json({ error: "O email deve ser um email válido" });
    ...
    default:
      logger.error(
        `Erro de validação desconhecido: ${error.details[0].message}`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
...
export const signUpValidate = (req, res, next) => {
  ...
  switch (error?.details[0].message) {
    case `"password" with value "${password}" fails to match the required pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,}$/`:
      logger.warn(
        `IP ${req.ip} tentou fazer cadastro com senha que não atende os requisitos de complexidade`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"name" length must be at least 3 characters long':
      ...
      return res
        .status(400)
        .json({ error: "O nome deve conter no mínimo 3 caracteres" });
    case '"name" length must be less than or equal to 50 characters long':
      ...
      return res
        .status(400)
        .json({ error: "O nome deve conter no máximo 50 caracteres" });
    case '"email" length must be at least 13 characters long':
...
export const changePasswordValidate = (req, res, next) => {
  ...
  switch (error?.details[0].message) {
    case `"newPassword" with value "${newPassword}" fails to match the required pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,}$/`:
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou mudar a senha para uma nova senha que não atende os requisitos de complexidade`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
      ...
      return res.status(400).json({
        error:
          "A nova senha deve conter no mínimo 8 caracteres, incluindo letras maiúsculas, minúsculas, números e caracteres especiais",
      });
    case '"password" is required':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou mudar a senha sem preencher a senha atual`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"newPassword" is required':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou mudar a senha sem preencher a nova senha`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"confirmNewPassword" is required':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou mudar a senha sem confirmar a nova senha`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"confirmNewPassword" must be [ref:newPassword]':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou mudar a senha com senhas não coincidentes`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"password" length must be less than or equal to 100 characters long':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou usar uma senha muito longa`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"newPassword" length must be at least 8 characters long':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou usar uma nova senha muito curta`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
      ...
      return res
        .status(400)
        .json({ error: "A nova senha deve conter no mínimo 8 caracteres" });
    case '"newPassword" length must be less than or equal to 100 characters long':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou usar uma nova senha muito longa`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
      ...
      return res
        .status(400)
        .json({ error: "A nova senha deve conter no máximo 100 caracteres" });
    case '"password" must only contain alpha-numeric characters':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou usar credenciais inválidas`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"password" is not allowed to be empty':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou deixar o campo de senha atual vazio`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"newPassword" is not allowed to be empty':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou deixar o campo de nova senha vazio`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"confirmNewPassword" is not allowed to be empty':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou deixar o campo de confirmação de nova senha vazio`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"newPassword" must only contain alpha-numeric characters':
      logger.warn(
        `O usuário ${req.session ? req.session.user : "Desconecido"} tentou usar uma nova senha com caracteres inválidos`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
      ...
      return res.status(400).json({
        error: "A nova senha deve conter apenas caracteres alfanuméricos",
      });
    case undefined:
      logger.info(
        `O usuário ${req.session ? req.session.user : "Desconecido"} passou na validação de mudança de senha`,
        {
          usuario: req.session ? req.session.user : "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
...
export const resetPasswordValidate = (req, res, next) => {
  ...
  switch (error?.details[0].message) {
    case `"newPassword" with value "${newPassword}" fails to match the required pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,}$/`:
      logger.warn(
        `Tentativa de reset de senha com nova senha que não atende os requisitos de complexidade`,
        {
...
export const requestResetPasswordValidate = (req, res, next) => {
  ...
  switch (error?.details[0].message) {
    ...
    case '"email" must be a valid email':
      logger.warn(
        `Tentativa de solicitação de reset de senha com email inválido`,
        {
          usuarioId: "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case '"email" length must be at least 13 characters long':
      logger.warn(
        `Tentativa de solicitação de reset de senha com email muito curto`,
        {
          usuarioId: "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
    ...
    case undefined:
      logger.info(
        `Validação de solicitação de reset de senha passou com sucesso`,
        {
          usuarioId: "Desconecido",
          ip: req.ip,
          rota: req.originalUrl,
