...
export const Auth = async (req, res, next) => {
  try {
    if (!req.session?.user) {
      ...
      return res
        .status(401)
        .json({ message: "Logue ou registre-se para acessar" });
    }
    ...
    const sessionVersion =
      typeof req.session.version !== "undefined" ? req.session.version : 0;
    ...
    if (!usuario.isVerified) {
      ...
      return res
        .status(401)
        .json({ message: "Acesso negado. Verifique sua conta." });
    }
