...
export const authenticationSlowDown = slowDown({
  ...
  handler: (req, res, next, options) => {
    logger.warn(
      `IP ${req.ip} excedeu o limite de requisições livres para a rota ${req.originalUrl}, aplicando atraso`,
      {
        usuario:
          req.session && req.session.user ? req.session.user.id : "Desconecido",
        ip: req.ip,
